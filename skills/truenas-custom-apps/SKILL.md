...
- To exclude YouTube Shorts, do not trust a channel-route boolean without comparing actual video IDs: a `true`/`false` probe can return identical sets. Test the YouTube long-form uploads playlist route `/youtube/playlist/UULF<channel-id-without-UC>` instead, with a bounded `limit=` query and route-specific `code=`; confirm it returns valid XML, expected channel title, and video GUIDs. For an in-place FreshRSS conversion, back up PostgreSQL, check the limited playlist's video IDs against existing entries before refreshing (older playlist pages can flood notifications), transactionally map native `yt:video:<id>` GUIDs to RSSHub's `https://www.youtube.com/watch?v=<id>` and update the existing feed URL, then refresh via `actualizeFeedsAndCommit(<id>)`. Verify feed identity, zero unexpected new entries/errors, and preserved unread/starred counts. Historical Shorts remain unless separately removed.
- The native YouTube Atom endpoint also accepts `https://www.youtube.com/feeds/videos.xml?playlist_id=UULF<channel-id-without-UC>` (15 recent long-form items). For RSSHub→native bulk conversion, resolve `@handle`/`/user/` to the canonical channel ID from YouTube's channel page, require each target Atom feed's video IDs to overlap existing entries, back up DB/OPML/ZIP, and update existing feed IDs transactionally. Normalize RSSHub watch-URL GUIDs to `yt:video:<id>` only when no native GUID already exists; preserve pre-existing duplicate rows and all read/starred states, then force-refresh each feed and require no unexpected new entries/errors. Old signed ntfy mark-read actions need a feed-scoped watch-URL→native-GUID fallback after HMAC verification. A podcast artwork hook that iterates `SimplePie\Item::get_item_tags()` must handle `null` (`$tags ?? []`) or native YouTube feeds generate warnings on every refresh.
...
- A PWA cannot cancel the system back gesture or turn it into a finger-dragged drawer. To open the left feeds before leaving, use a user extension that `pushState`s a same-document trap while the mobile overlay is closed (`max-width: 840px`, or a coarse/standalone aside that is hidden). On `popstate`, open `#aside_feed` with `toggle_aside_click(false)`, clear a Youlag inline `display:none`, and `stopImmediatePropagation` so Youlag does not also call `history.back()`. If the aside is already open, one further `history.back()` is the real previous page. Yield to Youlag `articleOpen`/`modalOpen` and hash overlays. Do not arm on desktop. FreshRSS's Settings dropdown is a `#dropdown-configure` anchor controlled by `.dropdown-target:target ~ .dropdown-menu`; a hash navigation can also emit `popstate`. Return early on a nonempty `location.hash` before consuming/continuing the trap, so tapping Settings keeps it open. Let Back clear the hash first, then let the following Back open feeds. Test the actual gear anchor, target selector, and two sequential Back events in a FreshRSS-like fixture; jsdom `getComputedStyle()` does not reliably recalculate dynamic `:target` rules.
...
- Match added control icons to the native theme pipeline: an inline white SVG without `.icon` bypasses theme filtering. Prefer the native `themes/icons/share.svg` in `img.icon`; compare computed icon filter/size and control opacity, surface, and border against adjacent reader controls in normal, hover, and focus-visible states. Compact Media Cards uses opacity `0.9` normally and `1` on hover/focus; reuse its surface variables rather than guessing a dimming value. A CSP-blocked inline `<style>` has `sheet === null`; use a DevTools-created stylesheet for previews and verify again after a clean page load from deployed assets.
- Recover an interrupted managed restart by reading app state and jobs first. If `app.stop` succeeded and no start is pending, call `app.start` on the existing app, then verify job SUCCESS, healthy app/database containers, HTTP, and served asset hashes. Do not reinstall. Static extension asset updates with changed cache-busting URLs need no app restart.
